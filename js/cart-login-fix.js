// Fixed cart functionality with login prompt for dashboard
document.addEventListener('DOMContentLoaded', function() {
  ...
  window.addToCart = function(product) {
    ...
    if (!user) {
      // Show login modal on dashboard if available
      const dashboardModal = document.getElementById('login-modal');
      if (dashboardModal) {
        // We're on the dashboard, show the modal
        dashboardModal.style.display = 'flex';
        return;
      } else {
        // Regular confirm dialog on other pages
