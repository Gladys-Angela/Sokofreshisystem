/**
 * This script fixes product visibility issues by ensuring all products have the isVisible field
 * and that the query properly filters for visible products
 */
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function loadProducts() {
    ...
    // Only show visible products or products without visibility field
    // This ensures backward compatibility with products that don't have the field yet
    ...
    query.get()
      .then((querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
          ...
          // Include products that are visible or don't have the visibility field
          if (productData.isVisible === undefined || productData.isVisible === true) {
            products.push({
              id: doc.id,
              ...productData
            });
        ...
        // Check if we have any visible products
        if (products.length === 0) {
          productContainer.innerHTML = `
            <div class="no-products-container">
              <div class="no-products-icon"><i class="fas fa-box-open"></i></div>
              <h3>No visible products found</h3>
              <p>We couldn't find any visible products matching your criteria.</p>
            </div>
          `;
          return;
        }
